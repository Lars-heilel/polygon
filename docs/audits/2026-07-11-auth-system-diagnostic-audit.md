...
| Gateway auth RPC error envelope | done | `GatewayHttpExceptionFilter` normalizes HTTP/RPC errors to `{ statusCode, error, message, path, timestamp }`; gateway HTTP tests prove unsafe 5xx downstream messages do not expose email/token data. | Swagger still needs to document the shared error envelope. |
...
| --- | --- | --- | --- |
| P0 | Document gateway error envelope in Swagger. | Runtime now returns a normalized `{ statusCode, error, message, path, timestamp }` envelope, but OpenAPI does not yet describe that shared error shape. | Generated Swagger assertions for representative auth error responses. |
