...
| Backend auth: password recovery | Forgot/reset password hides account enumeration and revokes sessions when required. | Unknown or passwordless accounts return normally from `forgotPassword`; reset hashes the new password, clears login-attempt state, and revokes sessions. The unauthenticated reset-token flow still has no current-session context to preserve even though `docs/specs/auth-service.md:206` says to retain the current authenticated session when applicable. | `libs/backend/auth/src/services/auth.service.spec.ts` | needs-decision | Decide whether reset-token password reset should always revoke every session, or whether an authenticated reset flow must pass `currentSessionId` and preserve that session. |
...
| Gateway auth: Swagger accuracy | Swagger docs match observed endpoint status codes, cookies, and response shapes. | Generated-document tests now cover auth message responses, verify-email redirect headers, and session response schemas, but several auth endpoints still have stale cookie security or missing message/cookie response schemas. | `apps/backend/gateway/src/controllers/auth.swagger.spec.ts` | bug | Finish the remaining endpoint-specific corrections listed in `Swagger Drift`. |
...
| Auth RPC errors are not normalized by a global error envelope | The controller maps an RPC status/message into `HttpException`, while the gateway specification expects a consistent global error response shape. | missing-from-code | Define and test the global error envelope before documenting it in Swagger. |
| OAuth callback security needs runtime confirmation | OAuth callbacks set cookies and redirect to configured `CLIENT_URL`; logger redaction tests cover `token` and `tokens`, but callbacks have no gateway tests or provider execution. | needs-manual-validation | Run GitHub and Google provider callbacks in non-production and inspect redirect and log output. |
...
| Swagger describes the wrong cookie credential for refresh-oriented endpoints and omits some response contracts | Global Swagger config declares only `access_token`; logout and refresh decorate that scheme even though code consumes `refresh_token`; register/login/reset still lack concrete message schemas. Session, verify-email, resend-verification, and forgot-password docs now have generated-document assertions. | bug | Correct the remaining OpenAPI security and response definitions; extend generated-document assertions. |
...
| --- | --- | --- | --- |
| Should reset-token password reset preserve a current authenticated session? | Reset-token flow currently has no `currentSessionId` input, clears login attempts, and revokes all sessions after password change. | Keep reset-token flow as revoke-all; or add a separate authenticated password-change/reset path that preserves current session. | Treat email reset as revoke-all unless product wants an authenticated password-change flow. |
...
| --- | --- | --- | --- |
| P0 | Define gateway RPC error envelope. | Auth RPC errors are still converted locally to `HttpException(message, status)` without a global response contract. | Gateway Supertest coverage for representative auth RPC failures plus generated Swagger docs once the envelope is chosen. |
| P1 | Finish remaining Swagger/OpenAPI drift. | Register/login/reset still miss concrete message schemas, login/refresh/logout cookie documentation is incomplete, and refresh/logout still advertise `access_token` while reading `refresh_token`. | Extend `auth.swagger.spec.ts` generated-document assertions for the remaining endpoints. |
| P1 | Decide reset-token session semantics. | Password reset now clears login attempts, but email reset still revokes all sessions because the request has no current-session context. | Product decision, then auth service unit coverage if an authenticated current-session-preserving flow is added. |
| P2 | Runtime-smoke auth delivery and cookies. | Static and automated tests cover service logic, but real SMTP templates/links and browser cookie behavior still depend on environment/proxy/HTTPS. | Browser smoke: register, verify email, forgot/reset password, login, refresh, logout, session revoke. |
...
| P3 | Correct Swagger/OpenAPI drift after behavior decisions. | Remaining docs should match the chosen RPC error envelope and reset/session semantics. | Generated OpenAPI snapshot or controller metadata tests. |
...
| --- | --- | --- | --- |
| Gateway integration | `@org/gateway` | RPC error responses still lack a normalized gateway envelope. | Add Supertest cases for representative auth RPC errors and assert status/body shape without leaking raw payload data. |
...
| Swagger | `@org/gateway` | Some auth endpoint docs still miss concrete schemas or use the wrong cookie scheme. | Extend `auth.swagger.spec.ts` for register/login/reset message schemas and login/refresh/logout cookie contracts. |
