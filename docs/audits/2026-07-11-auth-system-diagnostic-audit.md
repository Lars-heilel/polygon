...
| Gateway auth: HTTP endpoints and validation | Auth HTTP endpoints route through the gateway and use DTO/Zod validation. | `AuthGatewayController` routes register, login, verification, recovery, OAuth, and session operations to `AUTH_PATTERNS` at `apps/backend/gateway/src/controllers/auth.controller.ts:63-399`; body endpoints use DTO-specific `ZodValidationPipe` and `main.ts:37-38` installs the global pipe. | none | implemented-no-test | Add focused Supertest coverage for every auth HTTP route, DTO rejection, and expected RPC pattern. |
...
| Gateway auth: frontend error intake | Frontend error intake validates the payload and logs a sanitized event. | `FrontendErrorController.capture` uses `frontendErrorSchema.safeParse`, rejects invalid payloads, and logs only parsed data plus `eventType` at `apps/backend/gateway/src/controllers/frontend-error.controller.ts:8-22`. | `apps/backend/gateway/src/controllers/frontend-error.controller.spec.ts` | implemented | No action. |
...
| Item | Evidence | Status | Follow-up |
| --- | --- | --- | --- |
| Auth HTTP routes apply DTO-specific Zod validation but have no direct controller suite | `AuthGatewayController` uses `ZodValidationPipe` for register, login, resend, forgot, and reset; global validation is installed in `main.ts:37-38`. `apps/backend/gateway/src/controllers/` contains no `auth.controller.spec.ts`. | implemented-no-test | Add auth route and validation Supertest tests. |
...
| Frontend error intake validates and strips unrecognized payload fields before logging | `safeParse` gates the event, then only `result.data` is spread into the structured log; direct tests verify successful logging and invalid-payload rejection. | implemented | No action. |
...
| `rg -n "Controller\\('auth|Post\\(|Get\\(|Delete\\(|ApiOperation|ApiResponse|ApiCookieAuth|cookie|clearCookie|refresh_token|access_token|ZodValidationPipe|Logger|send\\(|catchError|Unauthorized|SwaggerModule|DocumentBuilder" apps/backend/gateway/src` | pass | Exit `0`; located auth routes/cookie operations, socket access-cookie parsing, global Zod/Swagger bootstrap, and gateway tests. |
| `sed -n '1,520p' apps/backend/gateway/src/controllers/auth.controller.ts` and required gateway source reads | pass | Exit `0`; inspected auth controller, frontend-error controller, chat socket gateway, bootstrap, and gateway module. |
| `sed -n '1,460p' apps/backend/gateway/src/controllers/admin.controller.spec.ts` and required gateway test reads | pass | Exit `0`; inspected admin, frontend-error, and socket tests; `apps/backend/gateway/src/controllers/` has no `auth.controller.spec.ts`. |
