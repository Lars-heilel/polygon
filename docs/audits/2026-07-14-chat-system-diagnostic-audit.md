...
| WebSocket: cookie authentication | Socket connection authenticates with HttpOnly `access_token` cookie and rejects missing/invalid tokens. | Gateway extracts `access_token` from cookie header and verifies it with `TokenService`; shared client socket uses `withCredentials` and `autoConnect: false`. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:48-67`, `libs/client/shared/src/lib/socket/socket.ts:3-5`. | `apps/backend/gateway/src/gateways/chat.socket-gateway.spec.ts` covers token verification and ban paths, but not a missing-cookie case. | implemented-no-test | Add explicit missing-cookie and invalid-token socket tests. |
| WebSocket: instant revoke/account state | Socket connect checks active ban marker and exposes `disconnectUser` for explicit session/account revocation. | `handleConnection` checks `BanMarkerRepository.findActiveMarker`; `disconnectUser` disconnects every registered socket. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:70-90`, `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:134-143`. | `chat.socket-gateway.spec.ts` covers active ban, ban-check failure, and `disconnectUser`. | implemented | Add manual validation for session revoke while a socket is connected. |
| WebSocket: room lifecycle | Joining a chat room should be scoped to authenticated participants. | `handleJoin` uses socket `userId`, calls `CHAT_PATTERNS.CHECK_MEMBERSHIP`, and only joins/emits online when true. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:146-176`. | Current socket spec covers a successful join indirectly for log redaction, but not unauthorized join/no-join behavior. | implemented-no-test | Add socket unit tests for unauthorized join and membership RPC errors. |
| WebSocket: message delivery | New messages should be emitted to the chat room and offline recipients should receive push. | `handleSendMessage` delegates membership enforcement to Chat RPC, broadcasts on success, and triggers offline push; HTTP send uses the same broadcast helper. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:195-240`, `apps/backend/gateway/src/controllers/chat.controller.ts:154-170`. | No direct test for `handleSendMessage` success/failure fanout; no e2e two-browser test. | implemented-no-test | Add socket unit tests and Playwright two-browser realtime e2e. |
| WebSocket: typing indicator | Typing events should be scoped to chat participants. | `typing:start` and `typing:stop` only check authenticated `userId`; they do not verify membership or whether the socket joined the room before emitting to `chat:${payload.chatId}`. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:312-328`. | No typing tests. | bug | Check membership or joined-room state before emitting typing events; add unauthorized typing tests. |
| WebSocket: presence | Online/offline state is emitted on first chat join and final socket disconnect. | `userChats` records joined chats; first join emits `user:online`; final disconnect emits `user:offline` to each joined room. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:104-128`, `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:166-176`. | No direct presence tests beyond log-redaction setup. | implemented-no-test | Add tests for multi-socket presence and manual two-browser validation. |
| Client socket lifecycle | Client should connect after auth, disconnect on logout, and avoid duplicate listeners. | `initSocketMiddleware` connects only on `isAuthenticated` false-to-true transition and registers manager cleanup; manager adds/removes listeners. Evidence: `apps/client/messenger/src/app/socket/socket-middleware.ts:6-21`, `apps/client/messenger/src/app/socket/chat-socket-manager.ts:43-54`. | No direct client socket tests. | needs-manual-validation | Verify bootstrap path where session is already authenticated before middleware subscription; add unit tests for connect/disconnect/listener cleanup. |
...
| Item | Evidence | Status | Follow-up |
| --- | --- | --- | --- |
| Cookie auth and ban checks are covered better than realtime fanout. | Socket spec covers token verification path, active ban disconnect, fail-closed ban marker errors, `disconnectUser`, and redacted logs. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.spec.ts:142-229`. | implemented | Keep coverage and add missing no-cookie/invalid-token cases. |
| Typing can be emitted by any authenticated socket to any chat room ID. | Typing handlers do not call `CHECK_MEMBERSHIP` and do not verify joined rooms. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:312-328`. | bug | Gate typing events with membership or server-side joined-chat state. |
| Socket message send preserves `fileCategory`, unlike HTTP send. | Socket `message:send` includes `fileCategory` in Chat RPC payload. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:215-228`. | implemented-no-test | Add regression tests for socket file message payload and align HTTP route. |
| Offline push skips online users by gateway in-memory socket map. | `triggerPushForOfflineRecipients` checks `isUserOnline` for each member and emits `NOTIFICATION_EVENTS.SEND_PUSH` only when offline. Evidence: `apps/backend/gateway/src/gateways/chat.socket-gateway.ts:243-309`. | implemented-no-test | Test push recipient selection for sender, online recipient, and offline recipient. |
| Client socket middleware may miss already-authenticated initial state. | Subscription reacts to transitions via `(isAuthenticated, wasAuthenticated)` and does not connect immediately if the store is already authenticated when middleware initializes. Evidence: `apps/client/messenger/src/app/socket/socket-middleware.ts:6-21`. | needs-manual-validation | Validate app bootstrap ordering; if reproducible, connect when initial state is authenticated. |
| Client chat socket manager deduplicates message cache inserts and removes listeners on cleanup. | `handleNewMessage` ignores message IDs already present; cleanup calls `socket.off` for every registered event. Evidence: `apps/client/messenger/src/app/socket/chat-socket-manager.ts:9-28`, `apps/client/messenger/src/app/socket/chat-socket-manager.ts:43-54`. | implemented-no-test | Add unit tests around duplicate message and cleanup behavior. |
...
| `rg -n "socket\|Socket\|connect\|disconnect\|join\|leave\|room\|typing\|message\|presence\|online\|offline\|cookie\|access_token\|session\|ban\|emit\|on\\(" apps/backend/gateway/src/gateways apps/client/messenger/src/app/socket libs/client/features/chat-socket/src libs/client/shared/src/lib/socket` | pass | Exit `0`; captured backend socket gateway, socket specs, client middleware, manager, hook, and shared socket config. |
| `sed -n '1,380p' apps/backend/gateway/src/gateways/chat.socket-gateway.ts` | pass | Exit `0`; captured cookie auth, ban check, room join/leave, message send, push, typing, and broadcast helpers. |
| `sed -n '1,320p' apps/backend/gateway/src/gateways/chat.socket-gateway.spec.ts` | pass | Exit `0`; captured ban enforcement, `disconnectUser`, safe logging, and partial join/push setup. |
| `sed -n '1,260p' apps/client/messenger/src/app/socket/socket-middleware.ts` | pass | Exit `0`; captured auth-state subscription connect/disconnect behavior. |
...
| --- | --- | --- | --- |
| Two-browser direct message delivery | Requires two authenticated browser sessions and real Socket.IO connection. | Gateway, chat-service, auth/user dependencies, client dev server, two seeded users. | pending |
| Typing indicator authorization and display | Requires interactive browser timing and socket fanout; current static audit found missing membership check. | Two users in the same chat plus one negative attempt from a non-member if possible. | pending |
