...
export function ResetPasswordForm({ onSubmit }: ResetPasswordFormProps) {
  ...
  if (!token) {
    return (
      <div className="space-y-4 text-center">
        ...
        <Link to={CLIENT_ROUTES.auth.forgotPassword} className="text-sm text-primary hover:underline">
          Request a new one
        </Link>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit((values) => onSubmit({ ...values, token }))} className="space-y-4" noValidate>
