...
describe('media prisma schema id and type contract', () => {
  it('contains no UUIDv4 defaults', () => {
    expect(schema).not.toContain('@default(uuid())');
  });

  it.each(['File', 'MediaReference'])('generates UUIDv7 for %s.id', (model) => {
  ...
  it.each([
    ['File', 'uploaderId'],
    ['File', 'chatId'],
    ['MediaReference', 'fileId'],
    ['MediaReference', 'ownerId'],
  ])('stores %s.%s as native UUID', (model, field) => {
    expect(fieldLine(modelBlock(model), field)).toContain('@db.Uuid');
  });

  it('stores file size as BIGINT', () => {
    expect(fieldLine(modelBlock('File'), 'size')).toContain('BigInt');
  });

  it('keeps the shared FileCategory enum', () => {
    expect(fieldLine(modelBlock('File'), 'category')).toContain('FileCategory');
    expect(schema).toContain('enum FileCategory');
  });

  it('maps every model to a snake_case table', () => {
    for (const model of ['File', 'MediaReference']) {
