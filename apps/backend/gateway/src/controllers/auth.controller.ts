...
export class AuthGatewayController {
  ...
  async register(@Body() dto: RegisterDto, @GetClientMetadata() metadata: ClientMetadata) {
    this.logger.log(`Processing registration request for email: ${dto.email}`);
    this.logger.debug({ dto, metadata }, 'Full registration payload context');

    await this.send(this.authClient.send(AUTH_PATTERNS.REGISTER, dto));

    this.logger.log(`Successfully processed registration for email: ${dto.email}`);
  ...
  async login(
    ...
    this.logger.verbose({ tokens }, 'Tokens generated for login session');
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    ...
    this.logger.verbose({ tokens }, 'New rotated tokens generated by microservice');
    this.setTokenCookies(res, tokens);
  ...
  async verifyEmail(
    ...
    this.logger.debug({ token, metadata }, 'Verify email state and client metadata');
    ...
    this.logger.verbose({ tokens }, 'Tokens generated post-verification');
    this.setTokenCookies(res, tokens);
  ...
  async resendVerification(@Body() dto: ResendVerificationDto) {
    this.logger.log(`Processing resend verification request for: ${dto.email}`);
    this.logger.debug({ dto }, 'Resend payload state');
    ...
    this.logger.log(`Verification link successfully resent to: ${dto.email}`);
  ...
  async forgotPassword(@Body() dto: ForgotPasswordDto) {
    this.logger.log(`Processing forgot-password request for: ${dto.email}`);
    this.logger.debug({ dto }, 'Forgot password request payload');

    await this.send(this.authClient.send(AUTH_PATTERNS.FORGOT_PASSWORD, { email: dto.email }));

    this.logger.log(`Processed forgot-password routine for: ${dto.email}`);
    return {
  ...
  async resetPassword(@Body() dto: ResetPasswordDto) {
    ...
    this.logger.debug({ token: dto.token }, 'Reset password token parameter context');
